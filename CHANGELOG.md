...
- **Uniform-Config migration (in progress)** — widgets are being migrated to the dual-style `Config` standard (`CLAUDE.md` rules 2/4/8): `WidgetConfig` + `new()`/`default_config()` + one-line setters + `view(config, …)` terminal, with each widget's `*_*` shortcuts kept at their existing signatures. **Migrated:** `button`, `badge`, `alert`, `kbd`, `label`, `separator`, `spinner`, `skeleton`, `avatar`, `breadcrumb`, `progress`, `pagination`, `card`, `item`, `empty`, `input`, `textarea`, `checkbox`, `switch`, `select`, `radio_group`, `slider`, `search`, `input_otp`, `native_select`, `button_group`, `input_group`, `scroll_area`, `collapsible`, `accordion`, `tabs`, `toggle`, `toggle_group`, `stepper`, `dialog`, `alert_dialog`, `sheet`, `drawer`, `popover`, `hover_card`, `tooltip`, `dropdown_menu`, `context_menu`, `command`, `menubar`, `navigation_menu`, `rating`, `carousel`, `time_picker`, `calendar`, `date_picker`, `data_table`, `table`, `field`, `sidebar`, `navigation_bar`, `timeline`, `tree_view`, `aspect_ratio`. **The uniform-Config migration is now COMPLETE across all 58 element-rendering widgets** (the web-component wrappers + toast + tooltip's `attr`/`side_attr` remain the documented exceptions — see below). Note: `calendar.today()` (the zero-arg current-date getter) was renamed to `calendar.today_date()` to free `today` for the config setter. **Sanctioned exceptions** (not element-from-options widgets, kept as-is): `toast` (a notification system — `Toast` data model + `new_toast`/`toaster`); `tooltip`'s `attr`/`side_attr` attribute helpers (they return `Attribute`, not `Element`; the tooltip *wrapper* is Config-migrated); and the **web-component / FFI wrappers** that bridge to an external JS library via a custom element and configure it through HTML attributes + native JS *properties* rather than rendering an element from a Gleam options record — `code_editor`, `monaco_editor`, `d3_bar_chart`, `lustre_bar_chart` (+ `lustre_bar_chart_helpers`), `lustre_heatmap`, `world_map`, `entity_graph_3d`, `entity_graph_canvas`, `canvas_command`, and `graph_layout`. These follow the separate **Web Component (External Library Wrapper) Rules** in `CLAUDE.md`, not the widget Config pattern. **Breaking:** flat constructors / `Attrs` types removed — `badge(label, variant)`, `alert(variant, title, description, icon)`, `avatar(source, size, class)` (→ `view(config, source)`), `breadcrumb(items, attrs)` + `BreadcrumbAttrs`/`default_attrs` (→ `new() |> separator(..) |> view(items)`), `progress(value, attrs)` + `ProgressAttrs`/`default_attrs` (→ `new() |> … |> view(value)`), `pagination(page, total, on_change, attrs)` + `PaginationAttrs`/`default_attrs` (→ `new() |> … |> view(page, total, on_change)`), `card(CardAttrs)` + `CardAttrs`/`default_card_attrs` (→ `new() |> title(..)/description(..)/footer(..) |> view(content)`), `item(...)` (→ `new() |> … |> view(title, description, href)`; empty href → `<div>`, non-empty → `<a>`), `empty(...)` (→ `new() |> … |> view(title, description, content)`), `input(type_, value, on_input, attrs)` + `InputExtraAttrs`/`default_extra_attrs` (→ `new() |> … |> view(value, on_input)`), `textarea(value, on_input, attrs)` + `TextareaExtraAttrs`/`default_extra_attrs` (→ `new() |> … |> view(value, on_input)`). The `InitValue`/`SyncValue` (and `InitChecked`/`SyncChecked`) value-binding ADTs are unchanged. Also migrated: `checkbox` (flat `checkbox(...)` + `ExtraAttrs` removed → `new() |> form_attr/help_text/… |> view(label, status)`), `switch` (`SwitchExtraAttrs` removed → `view(label, status, on_change)`), `select` (`SelectExtraAttrs` removed → `view(options, value, on_change)`), `radio_group` (`RadioGroupAttrs`/`default_attrs` removed → `view(options, value, on_change)`). The full **form-inputs group is now migrated**: `slider` (`SliderAttrs`/`default_attrs` → `view(value, on_input)`), `search` (`SearchAttrs`/`default_attrs`; the old positional `size` arg is now a setter → `view(value, on_input, on_clear)`), `input_otp` (`InputOtpAttrs`/`default_attrs` → `view(value, on_change)`), `native_select` (`NativeSelectAttrs`/`default_attrs` → `view(options, value, name, on_change)`). The final group of seven standard widgets is now migrated: `table` (`TableExtraAttrs`/`default_table_attrs` and the `extra_attrs:` arg removed → `new() |> caption(..)/add_class(..) |> view(headers, rows)`), `field` (`FieldAttrs`/`default_attrs` and the flat `field(..)` removed → `new() |> label/description/error/required/orientation/hint |> view(input)`), `sidebar` (`SidebarAttrs`/`default_attrs` and the 5-arg `sidebar(..)` removed → `new() |> side/variant/collapsible |> view(open, header, content, footer)`; structural sub-elements `sidebar_header`/`sidebar_group`/`sidebar_menu_item`/… stay flat), `navigation_bar` (`NavBarAttrs` and the 5-arg `nav_bar(..)` removed → `new() |> variant(..) |> view(logo, links, actions)`; `nav_bar_link` unchanged), `timeline` (`TimelineAttrs` removed → `new() |> add_class(..) |> view(items)`), `tree_view` (`TreeViewAttrs` removed → `new() |> add_class(..) |> view(items, open_ids, on_toggle, on_select)`), and `aspect_ratio` (`AspectRatioAttrs` folded into `AspectRatioConfig` → `new() |> add_class(..) |> view(ratio, content)`; the `aspect_ratio(ratio, content)` shortcut is unchanged). Shortcuts keep their signatures (`badge_*`, `alert_*`, `kbd`, `label`/`label_for`, `separator`/`separator_vertical`, `spinner`/`spinner_simple`, `skeleton*`, `avatar_*`, `breadcrumb_simple`, `table`/`table_simple`, `field_simple`, `sidebar_simple`, `nav_bar_simple`, `timeline_simple`, `tree_view_simple`, `aspect_ratio` — no caller change except `table_simple`, which dropped its trailing `extra_attrs:` argument).
