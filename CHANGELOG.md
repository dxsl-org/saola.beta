...
- **Badge dual-style Config** — `badge` migrated to the uniform pattern: `BadgeConfig` + `new()`/`default_config()` + `variant`/`add_class` setters + `view(config, label)` terminal, with the `badge_*` shortcuts kept. First widget migrated under the new uniform-Config standard (see `CLAUDE.md` / `docs/code-standards.md`). **Breaking:** the flat `badge(label, variant)` is removed — use a `badge_*` shortcut or `new() |> variant(..) |> view(label)`.
