...
- **Uniform-Config migration (in progress)** — widgets are being migrated to the dual-style `Config` standard (`CLAUDE.md` rules 2/4/8): `WidgetConfig` + `new()`/`default_config()` + one-line setters + `view(config, …)` terminal, with each widget's `*_*` shortcuts kept at their existing signatures. **Migrated:** `button`, `badge`, `alert`, `kbd`, `label`, `separator`, `spinner`, `skeleton`, `avatar`, `breadcrumb`. **Breaking:** flat constructors removed — `badge(label, variant)`, `alert(variant, title, description, icon)`, `avatar(source, size, class)` (→ `view(config, source)`), `breadcrumb(items, attrs)` + `BreadcrumbAttrs`/`default_attrs` (→ `new() |> separator(..) |> view(items)`). Shortcuts keep their signatures (`badge_*`, `alert_*`, `kbd`, `label`/`label_for`, `separator`/`separator_vertical`, `spinner`/`spinner_simple`, `skeleton*`, `avatar_*`, `breadcrumb_simple` — no caller change).
