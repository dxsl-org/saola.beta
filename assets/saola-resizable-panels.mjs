/**
 * saola-resizable-panels — drag-to-resize panel groups
 *
 * Usage (set via Gleam a.property):
 *   element.sizes    = [30.0, 70.0]     (percentage per panel)
 *   element.minSizes = [10.0, 10.0]     (minimum % per panel)
 *
 * Attribute:
 *   direction = "horizontal" | "vertical"  (default: "horizontal")
 *
 * Dispatches CustomEvent("resize", { detail: [sizes] }) on drag end.
 */
class SaolaResizablePanels extends HTMLElement {
  ...
  constructor() {
    ...
    this._sizes = []
    this._minSizes = []
    this._dragging = null // { handleIndex, startPos, startSizes }
    this._bound = {
  ...
  _applyLayout() {
    ...
    // If no sizes provided, distribute evenly
    const sizes =
  ...
  _onPointerMove(e) {
    ...
    // Clamp so they don't exceed 100% together
